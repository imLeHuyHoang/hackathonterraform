...
# Package Creator Lambda Function (keeping separate for future use)
resource "aws_lambda_function" "package_creator" {
  s3_bucket     = var.lambda_code_bucket_id
  s3_key        = var.lambda_deployment_package_key
  function_name = "${local.resource_prefix}-package-creator"
  role         = var.lambda_execution_role_arn
  ...
  tags = merge(local.common_tags, {
    Name    = "${local.resource_prefix}-package-creator"
    Purpose = "Create deployment packages from processed data"
  })
}
...
# Lambda permission for S3 to invoke package creator
resource "aws_lambda_permission" "s3_invoke_package_creator" {
  statement_id  = "AllowExecutionFromS3BucketProcessed"
  action        = "lambda:InvokeFunction"
  function_name = aws_lambda_function.package_creator.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = var.main_bucket_arn
}
...
resource "aws_cloudwatch_log_group" "package_creator_logs" {
  name              = "/aws/lambda/${aws_lambda_function.package_creator.function_name}"
  retention_in_days = 14

  tags = merge(local.common_tags, {
    Name    = "${local.resource_prefix}-package-creator-logs"
    Purpose = "Lambda function logs for package creation"
  })
}